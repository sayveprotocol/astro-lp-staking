...
pub fn instantiate(
    ...
    store_config(
        deps.storage,
        &Config {
            sayve_token: deps.api.addr_canonicalize(&msg.sayve_token)?,
...
pub fn withdraw(deps: DepsMut, env: Env, info: MessageInfo) -> StdResult<Response> {
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: deps.api.addr_humanize(&config.sayve_token)?.to_string(),
            msg: to_binary(&Cw20ExecuteMsg::Transfer {
                recipient: info.sender.to_string(),
                amount,
            })?,
...
pub fn migrate_staking(
    ...
    let mut state: State = read_state(deps.storage)?;
    let sayve_token: Addr = deps.api.addr_humanize(&config.sayve_token)?;

    // get gov address by querying sayve token minter
    let gov_addr_raw: CanonicalAddr = deps
        .api
        .addr_canonicalize(&query_sayve_minter(&deps.querier, sayve_token.clone())?)?;
    ...
    let remaining_sayve = total_distribution_amount.checked_sub(distributed_amount)?;
    ...
    Ok(Response::new()
        ...
        .add_attributes(vec![
            ...
            ("remaining_amount", &remaining_sayve.to_string()),
        ]))
}
...
pub fn query_config(deps: Deps) -> StdResult<ConfigResponse> {
    ...
    let resp = ConfigResponse {
        sayve_token: deps.api.addr_humanize(&state.sayve_token)?.to_string(),
